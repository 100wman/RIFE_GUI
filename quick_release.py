...
def generate_release():
    ...
    compile_ols = f'nuitka --standalone --mingw64 --show-memory --show-progress --nofollow-imports                             --windows-icon-from-ico="{ico_path}" --windows-product-name="SVFI CLI" --windows-product-version={ArgumentManager.ols_version} --windows-file-description="SVFI Interpolation CLI"             --windows-company-name="Jeanna-SVFI"  --follow-import-to=Utils,steamworks,skvideo,RIFE,RIFE_cpu,SuperResolution                  --output-dir=release\{tag_version}                           .\one_line_shot_args.py'
    ...
    compile_gui = f'nuitka --standalone --mingw64 --show-memory --show-progress --nofollow-imports  --plugin-enable=qt-plugins --windows-icon-from-ico="{ico_path}" --windows-product-name="SVFI"     --windows-product-version={ArgumentManager.gui_version} --windows-file-description="Squirrel Video Frame Interpolation" --windows-company-name="SVFI"         --follow-import-to=Utils,steamworks,QCandyUi,skvideo         --output-dir=release\{tag_version} --windows-disable-console .\RIFE_GUI_Start.py'
